  ...
  return (
    <section className="panel plan-editor">
      ...
      <div className="form-grid two-columns">
        ...
        <label>Target amount<MoneyInput value={target} onValueChange={setTarget} placeholder="12,000.00" /></label>
        <label>Current amount<MoneyInput value={current} disabled={Boolean(accountId)} onValueChange={setCurrent} /></label>
        <label>Monthly contribution<MoneyInput value={monthly} onValueChange={setMonthly} placeholder="500.00" /></label>
...
function DebtEditor({ accounts, debt, onDone }: { accounts: AccountsResponse; debt?: DebtItem; onDone: () => void }) {
  ...
  return <section className="panel plan-editor"><div className="panel-heading"><div><span className="eyebrow">{debt ? "Edit debt" : "New debt"}</span><h2>{debt ? debt.name : "Add a debt"}</h2></div><button className="button ghost" type="button" onClick={onDone}>Close</button></div><div className="form-grid two-columns">
    ...
    <label>Balance<MoneyInput value={balance} disabled={Boolean(accountId)} onValueChange={setBalance} /></label>
    <label>APR %<input inputMode="decimal" value={apr} onChange={(event) => setApr(event.target.value)} /></label>
    <label>Minimum payment<MoneyInput value={minimum} onValueChange={setMinimum} /></label>
    <label>Extra payment<MoneyInput value={extra} onValueChange={setExtra} /></label>
...
function ForecastTab({ data }: { data: ForecastResponse }) {
  ...
  return <>
    <section className="panel forecast-settings"><div><span className="eyebrow">Forecast assumptions</span><h2>Protect a cash floor</h2><p>Budget reserve keeps planned spending in the projection instead of pretending every unspent dollar is free.</p></div><label>Reserve balance<MoneyInput value={reserve} onValueChange={setReserve} /></label><label className="checkbox-row"><input type="checkbox" checked={includeBudget} onChange={(event) => setIncludeBudget(event.target.checked)} /> Include budget reserve</label><button className="button primary" type="button" onClick={() => mutation.mutate()}>Save</button></section>
