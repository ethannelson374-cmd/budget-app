...
function GoalStep({ goals, onSaved, onSkip, onBack }: { goals: FinancialGoalsResponse; onSaved: (saved: FinancialGoalsResponse) => void; onSkip: () => void; onBack: () => void }) {
  ...
  return <form className="onboarding-copy" onSubmit={(e) => { e.preventDefault(); if (name && target) save.mutate(); else onSkip(); }}><span className="eyebrow">Goals</span><h1 id="onboarding-title">Anything you're working toward?</h1><p>Add one savings goal now, or skip it. You can build a full debt and goal strategy later in Plan.</p>{save.isError && <div className="inline-alert">{errorMessage(save.error)}</div>}<div className="form-grid two-columns"><label>Goal name <span className="optional">Optional</span><input value={name} onChange={(e) => setName(e.target.value)} placeholder="Emergency fund" /></label><label>Target amount<MoneyInput value={target} onValueChange={setTarget} disabled={!name} placeholder="10,000.00" /></label><label>Already saved<MoneyInput value={current} onValueChange={setCurrent} disabled={!name} placeholder="0.00" /></label><label>Monthly contribution<MoneyInput value={monthly} onValueChange={setMonthly} disabled={!name} placeholder="0.00" /></label></div><WizardActions back={onBack} nextLabel={name ? "Create goal" : "Skip for now"} busy={save.isPending} disabled={Boolean(name && !target)} /></form>;
}
