...
| `REGISTRATION_MODE` | `open` enables public password/Google account creation; `invite_only` requires an invite for new accounts; `disabled` disables normal registration. Invitations continue to work in every mode. |
...
Phase 4 begins by replacing Budget's one-owner-only authentication assumptions with a small multi-user identity layer. The initial owner remains the administrator. Administrators can invite family members by email; accepting an invitation creates a fully owner-scoped Budget user with a fresh onboarding state and the existing default category catalog.
...
Google and SMTP are optional. With `EMAIL_DELIVERY=disabled`, the administrator can copy private invitation and password-reset links directly from Settings, which keeps the app useful for a small trusted family deployment without adding a mail provider. Public registration is controlled by `REGISTRATION_MODE`; use `open` in production to enable self-service accounts, while shared invitations remain the path for joining another user's Budget. Migration `20260814_0013` adds the Stage 1 identity/security records and marks the existing owner as the first administrator.
...
The Dashboard cash-flow widget now follows the Phase 5B Compact/Standard/Hero information-density model. Compact presents headline cash-flow figures, Standard renders a condensed flow map, and Hero exposes the full optical-glass Sankey with range controls, proportional volumetric flow tubes, prior-period comparisons, transaction counts, transaction drill-down, and an `Ask Budget` handoff for the selected flow. The Month/Year/Custom controls sit below the visualization in a stable interaction layer so dashboard hover depth does not interfere with pointer hit-testing, and the expanded stage spacing keeps high-volume ribbons clear of summary and inspector content. Visual depth is created with lighting, transparency, shadows, and specular layers while ribbon widths remain proportional to the underlying values. No new runtime chart dependency or database migration is required; the production schema remains `20260815_0020` until the coordinated Phase 5 release.


### Phase 5D — net worth and financial trends

Phase 5D adds a **Trends** view inside the consolidated Analytics workspace and a reusable owner-scoped `GET /api/v1/trends` model with 30-day, 3-month, 6-month, year-to-date, 1-year, and all-history ranges. The view combines existing aggregate financial snapshots with current account balances and transaction history to show net-worth movement, asset/liability composition, income-versus-spending history, category momentum, income-source concentration, and current account contribution. Transfers, pending transactions, and transactions explicitly excluded from spending are not allowed to contaminate spending/income trend math.

Migration `20260817_0021` adds `account_balance_snapshots`, captured by the existing reporting snapshot worker. These per-account daily observations let Budget measure which accounts actually moved net worth over time without reconstructing or inventing balances that were never observed. Aggregate net-worth history can immediately use the older `financial_snapshots` table; account-level contribution becomes richer as Phase 5D snapshots accumulate. The demo reset seeds several months of synthetic snapshot history only in the guarded local SQLite demo so the visualizations are useful during development.

The Trends UI extends Aurora Ledger with a mathematically accurate optical-glass **financial terrain**: a glowing net-worth ridge, wireframe depth plane, glass-orb inspection points, current asset/liability composition, tactile account-momentum meters, dimensional income/spending bars, transaction drill-down, and Ask Budget handoff. The existing Dashboard Net Worth widget now links into Analytics → Trends and expands into a small three-month ridge when resized beyond Compact. Visual depth is produced by lighting, shadows, transparency, and layered geometry; the plotted values themselves are never perspective-distorted.


### Phase 5E — recurring activity and financial calendar

Phase 5E turns the existing recurring-pattern detector into a forward-looking **Financial calendar** without pretending forecasts are posted transactions. `GET /api/v1/financial-calendar?month=YYYY-MM` combines owner-scoped recurring streams, known pending activity, and debts that have an explicit due day. Posted recurring activity is labeled **Posted**, provider/manual pending items are labeled **Pending**, pattern-derived occurrences are labeled **Expected**, and debt-strategy commitments with a real due day are labeled **Planned**. Budget does not invent due dates for goals, flexible budget reserves, or debts whose due day is unknown.

The calendar projects the user's aggregate cash path from current depository cash through dated expected/planned events, exposes the lowest projected balance and configured reserve pressure, and supports both a chronological timeline and spatial month grid. Pending transactions are visible as known activity but are not applied a second time to the projection because the account's available balance may already reflect provider holds. Every event can drill into matching Transactions and can hand a bounded descriptive prompt to Ask Budget.

Aurora Ledger's tactile material system carries into the calendar through a glowing projected-balance ribbon, glass event orbs, translucent day cells, and a selected-event inspector. The existing `/recurring` route remains as a compatibility alias while the primary navigation now uses `/calendar`. Reanalysis still uses the deterministic recurring detector; no new timer, external provider, database table, or Alembic migration is required, so the Phase 5 development schema remains `20260817_0021`.

### Phase 5F — release hardening and production readiness

Phase 5F adds no new financial feature. It converts the Phase 5 branch into a release candidate by adding a single secret-free release gate, route-level frontend code splitting, CI regression coverage, an explicit Phase 4-to-Phase 5 migration rehearsal, and a documented production rollback path.

`python -m app.cli release-readiness` combines database connectivity/schema checks, the Phase 5A security posture, Plaid Production readiness, scheduled-worker health, and a MySQL-specific guard that verifies provider identifiers still use the case-sensitive `utf8mb4_bin` collation required by Plaid transaction IDs. Local/demo rehearsals may warn about production-only configuration while still succeeding; the production cutover runs `release-readiness --require-production --strict-operations` and fails on stale/unverified workers as well as hard security/schema/Plaid findings.

The protected application routes are now loaded on demand with React `lazy`/`Suspense`, so opening a simple workspace no longer requires downloading every Sankey, Trends, Calendar, Reports, Advisor, and Settings implementation up front. The GitHub quality workflow runs the complete backend regression suite without the local coverage fail-under gate, plus frontend typecheck, Vitest, and production build on Node 24. `deploy/PHASE5_RELEASE.md` is the authoritative merge/deployment/rollback runbook, and `deploy/scripts/phase5-production-preflight.sh` verifies Nginx, local health/readiness, release readiness, required timers, and loopback-only Uvicorn binding on the E2 host.

### Phase 5G — invite links and first-run onboarding

Phase 5G closes the initial product build with a consumer-style first-run experience. Family access is no longer bound to an email address before registration: an administrator creates a private, one-use invite link with an optional friendly label. The raw invite credential is random, stored only as a digest, expires after seven days, and can be revoked before use. Opening `/join/<token>` exchanges that credential for a short-lived registration challenge and immediately removes the raw token from the browser URL. The recipient can then create a local account with their own email/username/password or continue through the existing Google OpenID Connect flow.

New invite-created users are not dropped directly onto the Dashboard. They enter a resumable Aurora Ledger onboarding flow for financial preferences, optional Plaid bank connection, an initial monthly/annual budget starting point, an optional savings goal, and Ask Budget privacy choices. Every financial step reuses the existing Budget/Plaid/Plan/Advisor APIs and can be skipped when it is not required. Protected application routes redirect incomplete new users back to onboarding until they explicitly finish. Migration `20260817_0022` marks all existing users complete at step 6 so current accounts are never forced through the wizard, while new invited accounts begin at step 0.

### Navigation consolidation

The primary navigation now favors financial jobs instead of one sidebar item per feature. **Budget** lives inside **Plan**, **Insights** and **Trends** share **Analytics**, and deterministic **Reports** live beside Ask Budget inside **Advisor**. The legacy `/budget`, `/insights`, `/trends`, and `/reports` routes remain compatibility redirects so saved links continue to land in the corresponding consolidated workspace. **Calendar** remains a first-class destination because scheduled and expected cash events are operational planning rather than retrospective analytics.
