...
## Phase 3D Stage 4 report/export boundary

Saved report configurations contain only a name, reporting range, and allowlisted section keys. They do not cache raw transaction rows. Export creation rebuilds the selected report through the same owner-scoped deterministic reporting services used by the UI, serializes that report payload for audit/reproducibility, renders the requested CSV or PDF server-side, and stores the exact generated bytes with a SHA-256 digest. Export downloads verify the stored bytes against that digest before returning them with `Cache-Control: private, no-store`.

CSV generation treats formula-like user text as text rather than executable spreadsheet formulas. PDF generation is local to the FastAPI process and requires no headless browser, Node process, cloud document service, or additional network access. Report/export mutation endpoints retain the application's authenticated CSRF and audit-event boundaries.

The Reports-to-Advisor handoff is intentionally identifier-only from the browser: `report_section` and `report_range` are accepted with the user question, and the backend regenerates the attached report from Budget's current deterministic services. The AI provider never receives client-authored report totals. Context is bounded to representative history/category rows, and the existing `advisor_share_merchants` preference is enforced again at this boundary before any merchant names can enter AI context.

## Phase 4 Stage 1 identity/security boundary

Budget remains intentionally small-scale and invite-only. `User` is still the ownership boundary for all financial records, while `is_admin` grants only family-access administration functions; it does not bypass owner scoping for another user's accounts, transactions, budgets, reports, Advisor history, or Plaid records. The existing installation owner is promoted to the first administrator during migration.

Authentication methods are modeled independently from the Budget user. Local passwords remain Argon2 hashes. Google identities are stored by provider plus Google's immutable subject identifier. Email equality alone is never sufficient to attach a Google identity to an existing user: linking requires an authenticated Budget session, while Google-first account creation requires a live invitation whose normalized email matches a provider-verified email claim. OAuth authorization state and nonce records are short-lived, one-use, digest-backed records with a constrained relative return path.

Invitation, password-reset, session, OAuth-state, and two-factor challenge bearer values are never persisted in plaintext. TOTP secrets and recovery-code material are encrypted per-user with AES-GCM using the existing server-held application encryption key and purpose-specific associated data. Recovery codes are one-use. TOTP protects local password sign-in; Google authentication remains a distinct external sign-in method and inherits the user's Google account protections.
...
## Phase 4 Stage 4 notification boundary

Financial notifications are owner-scoped persisted events generated by deterministic Budget
services. The worker refreshes the deterministic Insight Engine and converts only allowlisted
signals into alerts according to each user's preferences. Goal milestones and newly imported
large expenses are computed directly from owner-scoped financial records. Weekly and monthly
summaries are transaction aggregates; Gemini or another AI provider never decides whether an
alert should fire or supplies notification totals.

`notifications(user_id, fingerprint)` is unique, making scheduled scans idempotent. Dismissal
and read state are independent of the underlying insight status. Disabling in-app delivery
